...
authoritative without a flag day: the v4 path ships dormant-but-safe and is armed
(or disarmed) by a single switch — the `GH_BENCH_INGEST_ROLE_ARN` CI variable — so
it can be rolled back without a deploy.
→ `CONTRACT.md`.
